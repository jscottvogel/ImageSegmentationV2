...
def evaluate_interpolation(method):
    ...
    with torch.no_grad():
        for idx in indices:
            ...
            for c in range(10):
                probs_resized[c] = cv2.resize(p_blend[c], (orig_w, orig_h), interpolation=method)
...
def main():
    print("Evaluating interpolation methods on validation set...")
    methods = [
    ...
    for name, method in methods:
        score = evaluate_interpolation(method)
        print(f"Method: {name:15s} | Score: {score:.5f}")
