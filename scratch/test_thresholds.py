...
def main():
    ...
    # Check first 100 validation images to save time, still highly representative
    val_img_paths = val_img_paths[:100]
    val_msk_paths = val_msk_paths[:100]
    
    print(f"Running threshold experiment on {len(val_img_paths)} validation images...")
    
    thresholds = [0.0, 0.3, 0.5, 0.7, 0.9, 0.95, 0.99, 1.0]
    
    # Store predictions and targets in memory to make threshold evaluation instantaneous
    all_probs = []
    all_targets = []
    ...
    for thresh in thresholds:
        ...
        for probs, target in zip(all_probs, all_targets):
            # If thresh > 0, we check if Class 0 probability is below thresh.
            # If it is, we choose argmax over classes 1-9.
            if thresh == 0.0:
                ...
                # For pixels where class 0 is predicted, check if prob is < thresh
            ...
            target_safe = np.where(target == 255, 0, target)
        ...
        print(f"\nThreshold: {thresh:.2f}")
        print(f"  Class 0 Dice: {dice[0]:.4f}")
        print(f"  Class 1-9 Avg: {np.mean(dice[1:]):.4f}")
        print(f"  Mean Dice (All 10): {np.mean(dice):.4f}")
