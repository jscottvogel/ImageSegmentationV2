...
def compute_kaggle_dice_torch(pred_b, targets_b):
    # pred_b: shape [B, H, W]
    # targets_b: shape [B, H, W]
    B = pred_b.shape[0]
    valid_mask = (targets_b != 255)
    
    # Shape: [B, 10, H, W]
    pred_onehot = torch.zeros((B, 10, pred_b.shape[1], pred_b.shape[2]), dtype=torch.bool, device=pred_b.device)
    tgt_onehot = torch.zeros((B, 10, targets_b.shape[1], targets_b.shape[2]), dtype=torch.bool, device=targets_b.device)
    
    for c in range(10):
        pred_onehot[:, c] = (pred_b == c) & valid_mask
        tgt_onehot[:, c] = (targets_b == c) & valid_mask
        
    sum_pred = pred_onehot.sum(dim=(2, 3))
    sum_tgt = tgt_onehot.sum(dim=(2, 3))
    intersection = (pred_onehot & tgt_onehot).sum(dim=(2, 3))
    
    both_empty = (sum_pred == 0) & (sum_tgt == 0)
    either_empty = (sum_pred == 0) | (sum_tgt == 0)
    
    dice = (2.0 * intersection.float()) / (sum_pred.float() + sum_tgt.float() + 1e-6)
    
    dice = torch.where(both_empty, torch.ones_like(dice), dice)
    dice = torch.where(either_empty & ~both_empty, torch.zeros_like(dice), dice)
...
def main():
    ...
    def evaluate_config_torch(w_syn_arr, thresh_arr):
        ...
        if len(fallback_classes) == 0:
            fallback_classes = [2] # default fallback to class 2
        ...
        for i in range(0, num_images, batch_size):
            ...
            dice_b = compute_kaggle_dice_torch(pred_b, targets_b)
            total_dice_sum += dice_b.sum().item()
            
        return total_dice_sum / (num_images * 10.0)
        
    # Baseline score (Kaggle-aligned)
    ...
    print(f"\nKaggle-style Baseline Validation Dice (w_syn=0.50, c0=0.95, c1=0.50, c3=0.50): {baseline_dice:.6f}")
    
    # 3. Powell Optimization of Weights and Thresholds
    print("\nStarting Powell optimization targeting Kaggle-style Row-wise Dice...")
    ...
    # Run Powell method
    res = minimize(loss_function, initial_x, method='Powell', options={'maxiter': 8, 'disp': True})
    ...
    print(f"\nBest Validation Dice (No morphological cleanup): {optimized_dice:.6f} (Delta: {optimized_dice - baseline_dice:+.6f})")
    
    # 4. Sweep min_area Morphological Cleanup on CPU
    print("\nSweeping min_area post-processing parameter...")
    ...
    min_areas = [0, 8, 16, 24, 32, 48, 64, 80, 96, 128, 160, 192, 256, 384, 512]
    best_area = 0
    best_area_dice = 0.0
    ...
    for area in min_areas:
        ...
        # Clean up images sequentially on CPU, then calculate Kaggle Dice in batch on GPU
        cleaned_preds = np.zeros((num_val, 480, 640), dtype=np.uint8)
        for idx in range(num_val):
            cleaned_preds[idx] = neighbor_fill_cleanup(numpy_preds[idx].copy(), min_area=area)
            
        cleaned_t = torch.tensor(cleaned_preds, device=device)
        
        for i in range(0, num_val, batch_size):
            end_idx = min(i + batch_size, num_val)
            pred_b = cleaned_t[i:end_idx]
            targets_b = Targets[i:end_idx]
            
            dice_b = compute_kaggle_dice_torch(pred_b, targets_b)
            total_dice_sum += dice_b.sum().item()
            
        area_dice = total_dice_sum / (num_val * 10.0)
        print(f"min_area = {area:<3d} -> Validation Kaggle-style Dice: {area_dice:.6f}")
        
        if area_dice > best_area_dice:
            best_area_dice = area_dice
            best_area = area
            
    print(f"\nOptimal Morphological Area: {best_area} with Kaggle-style Dice score: {best_area_dice:.6f}")
    ...
    torch.save(config_dict, "model_checkpoint/ensemble_kaggle_config.pt")
    print(f"\nConfiguration saved successfully to model_checkpoint/ensemble_kaggle_config.pt")
