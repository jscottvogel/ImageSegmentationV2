...
def main():
    ...
    val_mask_paths = val_mask_paths[:150]
    num_val = len(val_image_paths)
    print(f"Total Validation Images: {num_val}")
    
    # Precomputing blended probabilities in memory (float16 on GPU)
    ...
    if os.path.exists(syn_weights):
        ...
        model_syn.load_state_dict(state, strict=True)
    model_syn.eval()
    ...
    # Set fallback classes based on thresh
    ...
    with torch.no_grad():
        for idx in tqdm(range(num_val), desc="Inference"):
            ...
            probs[0] = p_syn_np[0] # Class 0 unscaled
    ...
    # Initial area cuts (use 96 as baseline)
    current_areas = [96] * 10
    
    # Eval initial configuration
    print("\nEvaluating baseline (area=96 globally)...")
    ...
    print(f"Baseline macro Dice: {best_score:.6f}")
    ...
    while improved and iterations < max_iterations:
        ...
        # Class order: 7 (Pool), 8 (Vehicle), 3 (Water), 1 (Building), 2 (Road), 4 (Barren Land), 5 (Vegetation), 6 (Agricultural), 9 (Others), 0 (Background)
    ...
    print(f"Optimal Class-specific Morphological Areas:")
    for c in range(10):
        print(f"  Class {c}: {current_areas[c]}")
    print(f"Final Validation Dice: {best_score:.6f} (Delta over global 96: {best_score - config.get('final_dice_with_area', 0.944095):+.6f})")
    
    # Save optimized config
    config['best_areas'] = current_areas
    config['final_dice_with_class_areas'] = best_score
