...
def evaluate_blend(w_main, w_unet, w_deeplab, w_fcn, min_area=100):
    ...
    with torch.no_grad():
        for idx in indices:
            ...
            # Preserve Class 0 from main output
            p_blend[0] = p_main[0]
...
def main():
    print("Sweeping blend weights with Min Area 100...")
    blends = [
        ("1.0 Main Only", 1.0, 0.0, 0.0, 0.0),
    ...
    for desc, wm, wu, wd, wf in blends:
        score = evaluate_blend(wm, wu, wd, wf, min_area=100)
        print(f"Blend: {desc:45s} | Score: {score:.5f}")
