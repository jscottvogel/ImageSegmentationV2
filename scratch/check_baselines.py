...
# Use first 50 images for speed
val_images = val_images[:50]
val_masks = val_masks[:50]
print(f"Loaded {len(val_images)} validation images")

models = {
    'DeepLab Best Active': load_weights(CustomDeepLabV3Plus(num_classes=10).to(device), 'model_checkpoint/FloodNet_PyTorch/best_deeplab_weights.pt'),
    'DeepLab SWA Active': load_weights(CustomDeepLabV3Plus(num_classes=10).to(device), 'model_checkpoint/FloodNet_PyTorch/final_swa_smoothed_weights.pt'),
    'DeepLab Best Backup': load_weights(CustomDeepLabV3Plus(num_classes=10).to(device), 'model_checkpoint/FloodNet_PyTorch_V2S_Backup/best_deeplab_weights.pt'),
    'DeepLab SWA Backup': load_weights(CustomDeepLabV3Plus(num_classes=10).to(device), 'model_checkpoint/FloodNet_PyTorch_V2S_Backup/final_swa_smoothed_weights.pt'),
    'UNet Best Active': load_weights(StandardUNet(num_classes=10).to(device), 'model_checkpoint/FloodNet_UNet/best_unet_weights.pt'),
    'UNet SWA Active': load_weights(StandardUNet(num_classes=10).to(device), 'model_checkpoint/FloodNet_UNet/final_swa_smoothed_unet.pt'),
    'FCN Best Active': load_weights(ResNet50FCN(num_classes=10).to(device), 'model_checkpoint/FloodNet_FCN/best_fcn_weights.pt'),
    'FCN SWA Active': load_weights(ResNet50FCN(num_classes=10).to(device), 'model_checkpoint/FloodNet_FCN/final_swa_smoothed_fcn.pt')
}
