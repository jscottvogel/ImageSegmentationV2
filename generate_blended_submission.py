...
def main():
    ...
    import csv
    with open(out_t30, 'w', newline='') as f_t30, \
         open(out_t95, 'w', newline='') as f_t95, \
         open(out_t99, 'w', newline='') as f_t99:
         
        w_t30 = csv.writer(f_t30)
        w_t95 = csv.writer(f_t95)
        w_t99 = csv.writer(f_t99)
        
        w_t30.writerow(["IMG_ID", "EncodedString"])
        w_t95.writerow(["IMG_ID", "EncodedString"])
        w_t99.writerow(["IMG_ID", "EncodedString"])
        ...
        with torch.no_grad():
            for idx, img_path in enumerate(tqdm(test_images)):
                ...
                img_flipped = torch.flip(img_tensor.cpu(), dims=[3]).to(device)
                ...
                p_syn_main_np = p_syn_main.squeeze(0).numpy()
                ...
                p_syn_unet_np = p_syn_unet.squeeze(0).numpy()
                ...
                p_syn_dl_np = p_syn_dl.squeeze(0).numpy()
                ...
                for class_id in range(10):
                    w_t30.writerow([f"{filename}_{class_id:02d}", mask2rle((pred_t30 == class_id).astype(np.uint8))])
                    w_t95.writerow([f"{filename}_{class_id:02d}", mask2rle((pred_t95 == class_id).astype(np.uint8))])
                    w_t99.writerow([f"{filename}_{class_id:02d}", mask2rle((pred_t99 == class_id).astype(np.uint8))])
