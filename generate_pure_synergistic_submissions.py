...
def main():
    ...
    out_syn_t95 = "synergistic_pure_tta_t95_area25_submission.csv"
    out_syn_t90 = "synergistic_pure_tta_t90_area8_submission.csv"
    out_hybrid_t95 = "hybrid_pure_tta_t95_area25_submission.csv"
    
    import csv
    with open(out_syn_t95, 'w', newline='') as f_syn_t95, \
         open(out_syn_t90, 'w', newline='') as f_syn_t90, \
         open(out_hybrid_t95, 'w', newline='') as f_hybrid_t95:
         
        w_syn_t95 = csv.writer(f_syn_t95)
        w_syn_t90 = csv.writer(f_syn_t90)
        w_hybrid_t95 = csv.writer(f_hybrid_t95)
        
        w_syn_t95.writerow(["IMG_ID", "EncodedString"])
        w_syn_t90.writerow(["IMG_ID", "EncodedString"])
        w_hybrid_t95.writerow(["IMG_ID", "EncodedString"])
        ...
        with torch.no_grad():
            for idx, img_path in enumerate(tqdm(test_images, desc="Running Pure TTA Pipelines")):
                ...
                for class_id in range(10):
                    ...
                    w_syn_t95.writerow([f"{filename}_{class_id:02d}", enc_a])
                    # B
                    enc_b = mask2rle((pred_b == class_id).astype(np.uint8)) if class_id in classes_b else ""
                    w_syn_t90.writerow([f"{filename}_{class_id:02d}", enc_b])
                    # C
                    enc_c = mask2rle((pred_c == class_id).astype(np.uint8)) if class_id in classes_c else ""
                    w_hybrid_t95.writerow([f"{filename}_{class_id:02d}", enc_c])
                    
                del img_tensor, img_flipped, pred_a, pred_b, pred_c
                gc.collect()
                torch.cuda.empty_cache()
                
    print(f"Saved {out_syn_t95}")
    print(f"Saved {out_syn_t90}")
    print(f"Saved {out_hybrid_t95}")
