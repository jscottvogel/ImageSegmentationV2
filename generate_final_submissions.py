...
def main():
    ...
    with torch.no_grad():
        for idx, img_path in enumerate(tqdm(test_images, desc="Running Final Inference Pipelines")):
            ...
            for class_id in range(10):
                ...
                sub_syn_c0_c3.append([f"{filename}_{class_id:02d}", enc_1])
                # 2
                enc_2 = mask2rle((pred_2 == class_id).astype(np.uint8)) if class_id in classes_2 else ""
                sub_hybrid_c0_c3.append([f"{filename}_{class_id:02d}", enc_2])
                # 3
                enc_3 = mask2rle((pred_3 == class_id).astype(np.uint8)) if class_id in classes_3 else ""
                sub_hybrid_c0_c3_c1.append([f"{filename}_{class_id:02d}", enc_3])
    ...
    # Save CSVs
    pd.DataFrame(sub_syn_c0_c3, columns=["IMG_ID", "EncodedString"]).to_csv("synergistic_tta_suppress_c0_c3_submission.csv", index=False)
    print("Saved synergistic_tta_suppress_c0_c3_submission.csv")
    
    pd.DataFrame(sub_hybrid_c0_c3, columns=["IMG_ID", "EncodedString"]).to_csv("hybrid_tta_suppress_c0_c3_submission.csv", index=False)
    print("Saved hybrid_tta_suppress_c0_c3_submission.csv")
    
    pd.DataFrame(sub_hybrid_c0_c3_c1, columns=["IMG_ID", "EncodedString"]).to_csv("hybrid_tta_suppress_c0_c3_c1_submission.csv", index=False)
    print("Saved hybrid_tta_suppress_c0_c3_c1_submission.csv")
