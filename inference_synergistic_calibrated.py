...
def main():
    print(f"Starting Calibrated Inference Pipeline (Single-Pass Multi-Threshold)...")
    ...
    if os.path.exists(weights_path):
        ...
        print("CRITICAL ERROR: weights not found at model_checkpoint/FloodNet_Synergistic/best_synergistic_weights.pt")
        return
    ...
    for idx, img_path in enumerate(tqdm(test_images)):
        ...
        with torch.no_grad():
            ...
            # Common fallback (classes 1-9 argmax)
            ...
            # --- THRESHOLD 1.00 (Never Predict Class 0) ---
            pred_t100 = fallback.astype(np.uint8)
                
        # RLE encoding
        present_t95 = set(np.unique(pred_t95))
        present_t99 = set(np.unique(pred_t99))
        present_t100 = set(np.unique(pred_t100))
        ...
        for class_id in range(10):
            ...
            # t100
            if class_id not in present_t100:
                enc_t100 = ""
            else:
                enc_t100 = mask2rle((pred_t100 == class_id).astype(np.uint8))
            submission_t100_data.append([f"{filename}_{class_id:02d}", enc_t100])
            
        del img_tensor, fused_probs, probs_resized, pred_t95, pred_t99, pred_t100
        gc.collect()
        torch.cuda.empty_cache()
        
    out_t95 = 'synergistic_calibrated_t95_submission.csv'
    out_t99 = 'synergistic_calibrated_t99_submission.csv'
    out_t100 = 'synergistic_calibrated_t100_submission.csv'
    ...
    pd.DataFrame(submission_t100_data, columns=["IMG_ID", "EncodedString"]).to_csv(out_t100, index=False)
    
    print(f"SUCCESS! Calibrated submissions saved to:\n  - {out_t95}\n  - {out_t99}\n  - {out_t100}")
