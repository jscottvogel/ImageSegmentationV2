...
def main():
    ...
    for img_path in tqdm(test_images, desc="Running MST + TTA Unified Pipeline"):
        ...
        for class_id in range(10):
            ...
            deeplab_submission.append([f"{filename}_{class_id:02d}", encoded_dl])
        del labels_dl
        ...
        for class_id in range(10):
            ...
            unet_submission.append([f"{filename}_{class_id:02d}", encoded_unet])
        del labels_unet
        ...
        for class_id in range(10):
            ...
            fcn_submission.append([f"{filename}_{class_id:02d}", encoded_fcn])
        del labels_fcn
        ...
        for class_id in range(10):
            ...
            hybrid_submission.append([f"{filename}_{class_id:02d}", encoded_hybrid])
        del labels_hybrid
        ...
        if meta_layer is not None:
            ...
            for class_id in range(10):
                ...
                meta_submission.append([f"{filename}_{class_id:02d}", encoded_meta])
            del labels_meta
    ...
    # 5. Save all submissions
    print("\nSaving submissions...")
    pd.DataFrame(deeplab_submission, columns=["IMG_ID", "EncodedString"]).to_csv("deeplabv3plus_best_submission.csv", index=False)
    print("Saved deeplabv3plus_best_submission.csv")
    
    pd.DataFrame(unet_submission, columns=["IMG_ID", "EncodedString"]).to_csv("unet_best_submission.csv", index=False)
    print("Saved unet_best_submission.csv")
    
    pd.DataFrame(fcn_submission, columns=["IMG_ID", "EncodedString"]).to_csv("fcn_best_submission.csv", index=False)
    print("Saved fcn_best_submission.csv")
    
    pd.DataFrame(hybrid_submission, columns=["IMG_ID", "EncodedString"]).to_csv("hybrid_ensemble_submission.csv", index=False)
    print("Saved hybrid_ensemble_submission.csv")
    
    if meta_layer is not None:
        pd.DataFrame(meta_submission, columns=["IMG_ID", "EncodedString"]).to_csv("meta_ensemble_submission_tta.csv", index=False)
        print("Saved meta_ensemble_submission_tta.csv")
